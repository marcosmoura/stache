...
//! ## Usage
//!
//! Use the [`EventEmitter`] helper for type-safe event emission:
//!
//! ```ignore
//! let emitter = EventEmitter::new(app_handle);
//! emitter.emit(events::tiling::WORKSPACE_CHANGED, payload)?;
//! ```
...
/// Helper for emitting events to the Tauri frontend.
///
/// Provides a convenient wrapper around Tauri's event emission with
...
impl EventEmitter {
    ...
    pub fn emit<T: Serialize + Clone>(&self, event: &str, payload: T) -> Result<()> {
        self.app
            .emit(event, payload)
            .map_err(|e| super::error::Error::Other(format!("Failed to emit event: {e}")))
    }
    ...
    ///
    /// Returns an error if the event emission fails.
    pub fn emit_empty(&self, event: &str) -> Result<()> { self.emit(event, ()) }
}
...
/// Global app handle storage for event emission from contexts without direct access.
static GLOBAL_APP_HANDLE: Mutex<Option<AppHandle>> = Mutex::new(None);

/// Stores the app handle for global access.
///
/// This should be called once during app initialization.
pub fn set_global_app_handle(handle: AppHandle) {
    if let Ok(mut stored) = GLOBAL_APP_HANDLE.lock() {
        *stored = Some(handle);
    }
}

/// Gets a clone of the global app handle.
#[must_use]
pub fn get_global_app_handle() -> Option<AppHandle> {
    GLOBAL_APP_HANDLE.lock().ok().and_then(|guard| guard.clone())
}

/// Emits an event using the global app handle.
///
/// This is useful for emitting events from contexts where the app handle
/// is not directly available (e.g., callbacks, event handlers).
