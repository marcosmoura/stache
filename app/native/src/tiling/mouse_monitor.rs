//! Mouse event monitoring for the tiling window manager.
//!
...
//! allowing the tiling system to freeze event processing during these operations
//! and then reapply layouts appropriately when done.
//!
//! # How It Works
//!
//! The tiling system receives `AXWindowMoved` and `AXWindowResized` events
//! continuously while a window is being dragged/resized. Without knowing when
//! the drag ends, we can't properly respond to user-initiated moves/resizes.
//!
//! This module tracks:
//! - Mouse button down/up state
//! - Which allows us to detect: drag started (move event + mouse down) and
...
///
/// This callback is called once per mouse-up event, on the main thread.
...
pub fn init() -> bool {
    ...
    std::thread::Builder::new()
        .name("stache-mouse-monitor".into())
...
mod tests {
    ...
    fn test_initial_state() {
        // Mouse should not be down initially (unless running on CI)
    ...
    fn test_callback_management() {
        // Test that we can set and clear callbacks without panicking
        fn dummy_callback() {}
